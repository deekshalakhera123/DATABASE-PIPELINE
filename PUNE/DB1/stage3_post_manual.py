...
def classify_property_type(village_df: pd.DataFrame) -> pd.DataFrame:
    ...
    village_df['property_type'] = village_df['property_type_raw'].apply(map_property_type)
...
def assign_bhk_carpet_match(village_df: pd.DataFrame,
    ...
    def normalize_key(key):
        k = key.strip().upper()
        if k in NULL_KEYS:  return 'OTHERS'
        if k in SKIP_BHK:   return k
        return keyword_map.get(k, k)

    def clean_values(value_list):
        out = []
        for v in value_list:
    ...
    def find_closest_bhk(carpet, building_info):
    ...
    def process_row(i, carpet_raw, building_raw, parse_cache):
        ...
        cleaned = {k: clean_values(v) for k, v in building_info.items()}
        bhk = None
        ...
        if bhk is None:                             # Stage 2: closest match
            raw_bhk, _ = find_closest_bhk(carpet, cleaned)
        ...
        if bhk and bhk.upper() in SKIP_BHK:        # Stage 3: retry without skip types
            ...
            if filtered:
                new_bhk, _ = find_closest_bhk(carpet, filtered)
    ...
    print(f"Pre-parsing building info...")
...
def assign_bhk_range_fallback(village_df: pd.DataFrame,
    ...
    try:
        ...
        df2 = pd.DataFrame(phase_data_list)
        if df2.empty:
        ...
        df_expanded = pd.concat(df2.apply(reshape_data, axis=1).dropna().tolist(), ignore_index=True)
...
def run_stage3(city: str = CITY) -> pd.DataFrame:
    ...
    # 3.5 Geocode
    village_df = geocode_coordinates(merged_df, COORDINATES_PATH, city)
    ...
    village_df = assign_bhk_carpet_match(village_df, rera_grand, rera_keywords)
    village_df = assign_bhk_range_fallback(village_df, rera_grand, rera_keywords)
    village_df = finalise_bhk(village_df)

    # 3.8 Buyer location
    village_df = add_buyer_location(village_df, POSTAL_CSV_PATH)

    # 3.9 Final assembly
    result_df = final_assembly(village_df, lease_df, other_df, city, VILLAGE_DIR_PATH)
